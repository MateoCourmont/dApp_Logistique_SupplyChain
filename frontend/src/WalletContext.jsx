...
export const WalletProvider = ({ children }) => {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:3001/api/user/${address}`);
  ...
  useEffect(() => {
    if (!isLoadingRole) { // On attend que l'API ait répondu avant de décider
  ...
  return (
    <WalletContext.Provider value={{ account, role, connectWallet, disconnectWallet, isPopupVisible, setIsPopupVisible }}>
      {children}
